...
const IL_STYLES = `
  ...
  .il-tab--active { background: var(--panel-strong); color: var(--ink); box-shadow: 0 2px 8px rgba(23,48,66,0.08); }

  /* who note */
  .il-who-note {
    padding: 12px 16px; border-radius: 14px; font-size: 0.84rem;
    background: var(--accent-soft); color: var(--accent);
    border: 1px solid rgba(15,118,110,0.18); line-height: 1.55;
  }
  ...
  .il-card--open { box-shadow: 0 8px 24px rgba(23,48,66,0.10); }
  .il-card-header {
  ...
  .il-progress-bar { flex: 1; height: 8px; background: #e7eff1; border-radius: 999px; overflow: hidden; }
  .il-progress-fill { height: 100%; background: linear-gradient(90deg, var(--accent), #49a99f); border-radius: inherit; transition: width 300ms ease; }
  .il-progress-label { font-size: 0.8rem; font-weight: 700; color: var(--muted); flex-shrink: 0; }

  .il-clue-card {
    background: linear-gradient(135deg, rgba(218,243,236,0.5), rgba(255,255,255,0.95));
    border: 1px solid rgba(15,118,110,0.18); border-radius: 20px;
    padding: 24px 26px;
    box-shadow: 0 6px 18px rgba(23,48,66,0.07);
  }
  ...
  .il-results-score-ring {
    ...
    background: conic-gradient(var(--accent) calc(var(--pct) * 1%), #e7eff1 0%);
