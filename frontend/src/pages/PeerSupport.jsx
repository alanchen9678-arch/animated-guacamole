...
const MOD_RULES = [
  ...
  },
  {
    ...
    color: '#7c3aed', bg: 'rgba(124,58,237,0.06)', border: 'rgba(124,58,237,0.22)',
  },
  {
  ...
  return (
    <section className="page">
      <div className="ps-hub-identity">
        <AnonAvatar name={anonName} color="#0f766e" size={42} />
        <div>
          ...
          <div key={day}>
            ...
            {messages.filter(m => m.day === day).map(m => (
              <div key={m.id} className={`ps-msg-row${m.self ? ' ps-msg-row--self' : ''}`}>
                ...
                {m.self && <div className="ps-self-dot" style={{ background: '#0f766e' }}>{anonName.slice(0, 2)}</div>}
              </div>
            ))}
          </div>
        ))}
        ...
        {messages.map(m => {
          ...
          return (
            <div key={m.id} className={`ps-msg-row${isMe ? ' ps-msg-row--self' : ''}`}>
              ...
              {isMe && <div className="ps-self-dot" style={{ background: '#0f766e' }}>{anonName.slice(0, 2)}</div>}
            </div>
          )
        })}
        {isTyping && (
...
const PS_STYLES = `
  ...
  .ps-hub-card:hover { transform: translateY(-2px); box-shadow: 0 12px 28px rgba(23,48,66,0.10); }
  ...
  .ps-hub-card-icon--purple { background: rgba(124,58,237,0.10); color: #7c3aed; }
  .ps-hub-card-icon--teal   { background: rgba(15,118,110,0.10); color: var(--accent); }
  ...
  .ps-bubble-time  { font-size: 0.66rem; color: rgba(23,48,66,0.35); align-self: flex-end; }
