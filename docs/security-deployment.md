# Dawn Harbor production security checklist
...
- If the Supabase Data API is not used, revoke `anon` and `authenticated` grants on Dawn Harbor-owned tables and do not publish a service-role or secret key.
- Enable Network Restrictions for Postgres and Supavisor. Allow only the Dawn Harbor API service's Render outbound ranges, or dedicated outbound IPs.
...
- Apply the static-site headers in `render.yaml` to the live `dawn-harbor-web-qrql` service.
...
- Before storing regulated PHI, confirm whether Dawn Harbor is a covered entity or business associate and obtain legal/compliance review.
