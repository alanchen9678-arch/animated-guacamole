...
1. React stores the DRF token in browser `sessionStorage` and sends it as `Authorization: Token …`. Expired sessions return users to a login dialog without deleting recoverable drafts.
